...
public class Factura extends javax.swing.JPanel {
    ...
    public void cargarProductos() {
        ...
        ResultSet r = c.consultar("SELECT * FROM public.facturas");

        model = (DefaultTableModel) jTable1.getModel();
        try {
            while (r.next()) {
                model.addRow(new Object[]{r.getString("correlativo"), r.getString("nombre"), r.getString("fecha"),
                    r.getString("total")});
    ...
    public void generarPDF() throws FileNotFoundException {
        ...
        nestedtable.addCell(new Cell().add("INFORME: ").setBold().setBorder(Border.NO_BORDER));
        nestedtable.addCell(new Cell().add("FACTURAS").setBold().setBorder(Border.NO_BORDER));
